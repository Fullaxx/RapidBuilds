...
verbosecopymodule ${PACKAGESDIR}/nvidia_driver/011-nvidia-470.63.01-${KERNVERS}.xzm ${BASEDIR}/

# Chrome needs RDL
if [ "${INCCHROME}" == "Y" ]; then
  verbosecopymodule ${PACKAGESDIR}/chrome/chrome-92.0.4515.159-x86_64-1.xzm ${MODSDIR}/
fi

# VSCode needs RDL
if [ "${INCVSCODE}" == "Y" ]; then
  verbosecopymodule ${PACKAGESDIR}/vscode/vscode-1.57.1-${ARCH}-1.xzm ${MODSDIR}/
fi
...
verbosecopymodule ${PACKAGESDIR}/VirtualBox/VirtualBox-6.1.26-x86_64-1.xzm ${MODSDIR}/
...
verbosecopymodule ${PACKAGESDIR}/vivaldi/vivaldi-4.1.2369.18-${ARCH}-1.xzm ${MODSDIR}/
