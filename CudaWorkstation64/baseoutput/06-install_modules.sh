...
# Chrome needs RDL
if [ "${INCCHROME}" == "Y" ]; then
  verbosecopymodule ${PACKAGESDIR}/chrome/chrome-89.0.4389.128-x86_64-1.xzm ${MODSDIR}/
fi

# VSCode needs RDL
if [ "${INCVSCODE}" == "Y" ]; then
  verbosecopymodule ${PACKAGESDIR}/vscode/vscode-1.54.3-${ARCH}-1.xzm ${MODSDIR}/
fi
...
verbosecopymodule ${PACKAGESDIR}/vivaldi/vivaldi-3.7.2218.58-${ARCH}-1.xzm ${MODSDIR}/
