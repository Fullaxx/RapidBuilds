...
verbosecopymodule ${PACKAGESDIR}/nvidia_driver/011-nvidia-440.100-${KERNVERS}.xzm ${BASEDIR}/

# Chrome needs RDL
if [ "${INCCHROME}" == "Y" ]; then
  verbosecopymodule ${PACKAGESDIR}/chrome/chrome-84.0.4147.135-x86_64-1.xzm ${MODSDIR}/
fi

# VSCode needs RDL
if [ "${INCVSCODE}" == "Y" ]; then
  verbosecopymodule ${PACKAGESDIR}/vscode/vscode-1.48.2-${ARCH}-1.xzm ${MODSDIR}/
fi
...
verbosecopymodule ${PACKAGESDIR}/VirtualBox/VirtualBox-6.1.14-x86_64-1.xzm ${MODSDIR}/
...
verbosecopymodule ${PACKAGESDIR}/vivaldi/vivaldi-3.3.2022.47-${ARCH}-1.xzm ${MODSDIR}/
