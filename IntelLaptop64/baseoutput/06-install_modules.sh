...
# Chrome needs RDL
if [ "${INCCHROME}" == "Y" ]; then
  verbosecopymodule ${PACKAGESDIR}/chrome/chrome-75.0.3770.142-x86_64-1.xzm ${MODSDIR}/
fi

# VSCode needs RDL
if [ "${INCVSCODE}" == "Y" ]; then
  verbosecopymodule ${PACKAGESDIR}/vscode/vscode-1.35.1-${ARCH}-1.xzm ${MODSDIR}/
fi
...
verbosecopymodule ${PACKAGESDIR}/chromium/chromium-75.0.3770.142-${ARCH}-1alien.xzm ${MODSDIR}/
...
verbosecopymodule ${PACKAGESDIR}/docker/docker-18.09.8-${ARCH}-1.xzm ${MODSDIR}/
...
verbosecopymodule ${PACKAGESDIR}/vivaldi/vivaldi-stable-2.6.1566.49-1.${ARCH}.xzm ${MODSDIR}/
verbosecopymodule ${PACKAGESDIR}/vivaldi-codecs-ffmpeg-extra/vivaldi-codecs-ffmpeg-extra-74.0.3729.169-${ARCH}-1.xzm ${MODSDIR}/
